...
- кнопка с названием раздела на правой странице открывает **полноэкранный “манускрипт”** с деревьями рецептов выбранного раздела;
...
- для некоторых токенов/валют в составе показываются круглые иконки из `assets/*.png` (NEAR/HOPE/BEES/MED/Golden DarAi/Magic Dust);
...
| `boost` | Boost (опционально), строка вроде `+50%`. |
| `repFrom` | Репутация “до” (опционально), число. |
| `repTo` | Репутация “после” (опционально), число. |
| `pl` | PL (опционально), число. |
...
Рецепты подгружаются через `fetch("recipes.json")`, а **`main.js`** подключается как **ES-модуль** и импортирует `js/manuscriptTechTree.js`. При открытии **`index.html` как файла** (`file://`) браузер часто **блокирует** и JSON, и импорт модулей — страница не заработает как надо. Поэтому проще всего поднять маленький сервер:
